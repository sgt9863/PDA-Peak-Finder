...
def test_two_overlapping_peaks_separated():
    ...
    peaks = list(detect_peaks_deconvolved(ch, DeconvolutionConfig(min_prominence=0.02)))
    assert len(peaks) == 2
...
def test_gauss_and_emg_models_both_work():
    ...
    for model in ("emg", "gauss"):
        peaks = list(detect_peaks_deconvolved(
            ch, DeconvolutionConfig(model=model, min_prominence=0.02)))
        assert len(peaks) == 2
...
def test_resolved_peaks_not_merged():
    ...
    peaks = list(detect_peaks_deconvolved(ch, DeconvolutionConfig(min_prominence=0.05)))
    rts = sorted(p.apex_time for p in peaks)
    assert len(peaks) == 2
...
def test_return_components_shape():
    ...
    table, components = detect_peaks_deconvolved(
        ch, DeconvolutionConfig(min_prominence=0.02), return_components=True)
